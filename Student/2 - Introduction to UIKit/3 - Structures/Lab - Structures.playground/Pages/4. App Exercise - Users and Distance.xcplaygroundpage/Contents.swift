...
var reece = User(name: "Reece", age: 17, height: 71, weight: 135, activityLevel: 9)
print("\(reece.name) is \(reece.age)")
/*:
...
struct Distance {
    ...
    init(meter : Double) {
        self.meter = meter
        feet = self.meter / 3.28084
    }
    init(feet : Double) {
        self.feet = feet
        meter = self.feet * 3.28084
    }
}
//:  Now create an instance of `Distance` called `mile`. Use the initializer for meters to set the distance to 1600. Print out the property for feet and verify that it is equal to 5249.344.
let mile = Distance(feet: 1600)
print(mile)

//:  Now create another instance of `Distance` and give it some other distance. Ensure that both properties are set correctly.
let measurement = Distance(meter:5000)
print(measurement)
